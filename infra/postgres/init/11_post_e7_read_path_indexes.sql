-- E8 (partial): additive indexes closing 3 of the 5 O(n) read-path findings
-- from E6 load testing (docs/e6-results/E6_PERFORMANCE_REPORT.md sections 6
-- and 8, backlog items 2/3/5). See ADR-042. Purely additive: no RLS, grant,
-- or application-code changes -- query results are unaffected, only the
-- plan Postgres chooses.
